...
class SubjectEngine:
    ...
    def step(self, event: Event, *, advance_time: bool = True, choose_conduct: bool = True) -> ExpressionPacket:
        if advance_time:
        ...
        intention = self._choose_intention(event, needs, pressures) if choose_conduct else Action.WAIT
    ...
    def load(cls, path: str | Path, cartridge: Cartridge) -> "SubjectEngine":
        ...
        return cls.from_dict(raw, cartridge)

    @classmethod
    def from_dict(cls, raw: dict, cartridge: Cartridge) -> "SubjectEngine":
        """Restore the same subject from a host-owned atomic snapshot."""
        state = SubjectState(
    ...
    def _choose_intention(self, event: Event, needs: list[tuple[str, float]], pressures: list[tuple[str, float]]) -> Action:
        ...
        # Habits may choose within the current dominant channel, not override
        # a stronger incompatible concern merely because a weak need matches.
        if habit and habit.action in options and habit.strength >= 0.65 and self.state.tick - habit.last_used_tick > habit.cooldown:
    ...
    def _make_packet(self, owned: OwnedEvent, intention: Action, needs: list[tuple[str, float]], pressures: list[tuple[str, float]], leak_bucket: str) -> ExpressionPacket:
        memories = tuple(m.meaning for m in self._memories_by_ids(owned.matched_memory_ids))
        ...
        private = {"owned_meaning": owned.first_person_meaning, "self_relevance": owned.self_relevance, "surprise": owned.surprise, "sensorium": dict(self.state.sensorium), "matched_memory_ids": owned.matched_memory_ids}
