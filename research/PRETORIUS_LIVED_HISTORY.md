...
The next scientific step is now frozen as `research/PRETORIUS_LIVE_PROTOCOL_V1.md`. It uses a clean 2x2 intervention: explicit identity context on or off crossed with the full typed prehistory on or off, while the Pretorius cartridge and model are held constant. Because that artifact includes static canon, autobiography, phenotype evidence and longitudinal records, V1 tests prehistory utilization rather than isolating the causal effect of lived interaction. A LoRA "phenotype-only" arm is deliberately excluded from V1 because an actual adapter changes model parameters and requires a separate adapter-controlled protocol.
